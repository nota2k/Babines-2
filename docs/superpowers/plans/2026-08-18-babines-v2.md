...
| `YT_Babines/getAllPlaylists` | `youtube` | oui | `{title, itemCount, id, thumbnail}` — **`limit: 10`** |
| `YT_Babines/getLikedVideos` | `getyoutubeplaylists` | **non** | inactif et sans nœud de réponse — inexploitable |
| Deezer | — | — | **inexistant** |
...
Deux limites codées en dur tronquent silencieusement les données :

- `SPOTIFY_Babines/playlistTracks`, nœud `Spotify2` : `"limit": 20` — toute playlist est tronquée à
  ses 20 premiers morceaux.
- `YT_Babines/getAllPlaylists`, nœud YouTube : `"limit": 10` — seules 10 playlists YouTube sont
  listées, donc toutes les autres sont invisibles à l'import.

C'est incompatible avec la promesse centrale du produit — « rapatrier l'intégralité de ses
playlists ». Activer `returnAll` dans les deux cas.

- [ ] **Étape 3 : Décider du sort de `YT_Babines/getLikedVideos`**

Le workflow existe mais il est **inactif**, sans nœud de réponse, et déposé sur le chemin
`getyoutubeplaylists` — un nom qui ne correspond pas à ce qu'il fait. En l'état il n'est pas
exploitable : la tâche 8 traite donc YouTube comme une plateforme sans favoris (`liked: null`).

Deux issues acceptables : le terminer (nœud de réponse, activation, chemin renommé en
`youtube/liked`), puis basculer `ENDPOINTS.youtube.liked` dans `src/stores/import.js` ; ou le
supprimer pour qu'il cesse d'induire en erreur. Ne pas le laisser dans cet entre-deux.

Note : la liste des playlists YouTube, elle, existe bien et fonctionne — c'est
`YT_Babines/getAllPlaylists` sur le chemin `youtube`, et la tâche 7bis lit désormais sa forme réelle.
...
- [ ] **Étape 1 : Provisionner CouchDB — sans publier ses identifiants**

**Avertissement de sécurité, relevé à la revue finale de branche.** Vite *inline* toute variable `VITE_*` dans le JavaScript construit. Mettre les identifiants CouchDB dans `VITE_COUCHDB_URL` reviendrait à les publier en clair sur `babines.incongru.org` : n'importe quel visiteur obtiendrait un accès **lecture et écriture** à toute la bibliothèque. Ne le faites pas.

Trois issues, à trancher avant de déployer :

1. **Un utilisateur CouchDB dédié au mot de passe assumé public**, n'ayant droit qu'à cette seule base, avec CORS restreint à l'origine de l'application. Le plus simple : le mot de passe cesse d'être un secret et devient une clé d'accès à une base mono-utilisatrice.
2. **Un proxy d'authentification devant CouchDB** (cookie de session, jeton court). Plus sûr, plus de travail.
3. **Rester 100 % local pour l'instant** : laisser `VITE_COUCHDB_URL` vide. Le code le gère déjà proprement — l'indicateur affiche « local uniquement » et rien ne casse. La synchronisation entre appareils attend, tout le reste fonctionne.

`VITE_COUCHDB_DB` n'est pas un secret et peut être renseignée dans tous les cas.
